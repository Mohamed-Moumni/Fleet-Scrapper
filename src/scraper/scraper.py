...
class Scraper:
    ...
    def scrap_car_models(self, makes:List[Dict[str, str]]) -> None:
        ...
        for make,_ in zip(makes, tqdm(range(len(makes)), desc="Makes Treated")):
            ...
            models:List[Dict[str, str]] = self.main_frame.locator("#b > option:not([value='1'])").evaluate_all("""elements => elements.map(element => ({text:element.textContent.trim(), value: element.value}))""")
            models.pop(0)
    ...
    def scrap_car_sub_models(self, models:List[Dict[str, str]]) -> None:
        ...
        for model in models:
            ...
            sub_models = self.main_frame.locator("#c > option:not([value='1'])").evaluate_all(
                """
